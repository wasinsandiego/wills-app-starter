...
export const mergeEntity = (state, entity, newEntityState = {}, isBatch = false) => {
  ...
  // NOTE: If it's nil and it's for a batch then we create an empty Map instead of using
  // `initialEntityState` directly, i.e. `{ data, loading, error }`. The empty Map will be
  // populated with batchItemKeys that will include the initial state.
  const curEntityState = state.has(entityKey)
    ? state.get(entityKey)
    : isBatch ? fromJS({ }) : initialEntityState
  const entityState = curEntityState.merge(newEntityState)
  return state.mergeIn([entityKey], entityState)
}
...
export const applyCustomMerge = (state, payload, configs) => {
  ...
  const result = merge(get(state, [entity, 'data']), fromJS(data))
